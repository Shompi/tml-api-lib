...
export type APICoachingTicketCreationData = Partial<
  Omit<APICoachingTicket, "id">
> & {
  student_id: string;
  ingame_name: string;
};
...
const Routes = {
  Coach: {
    ...
    getTickets: (coach_id: string) =>
      `${API_URL}/api/coaching/coach/${coach_id}/tickets` as const,
  ...
  Student: {
    ...
    getTickets: (id: string) =>
      `${API_URL}/api/coaching/students/${id}/tickets` as const,
    /** This endpoint needs authentication */
  ...
  Tickets: {
    ...
    /** This endpoint needs authentication */
    claim: (id: string, coach_id: string) =>
      `${API_URL}/api/coaching/tickets/${id}/claim?coachId=${coach_id}` as const,
    /** This endpoint needs authentication */
...
export const CoachingAPI = {
  coaches: {
    ...
    getTickets: async (coach_id: string) => {
      if (!coach_id)
        throw new Error("You must provide a coach id to this function.");

      const response = await fetch(Routes.Coach.getTickets(coach_id), {
        method: "GET",
      });
    ...
    create: async (token: string, data: APICoachCreationData) => {
      if (!token)
        throw new Error(
          "You must provide an authorization token to this function.",
        );
  ...
  students: {
    ...
    getTickets: async (id: string) => {
      ...
      const response = await fetch(Routes.Student.getTickets(id), {
        method: "GET",
      });
  ...
  tickets: {
    get: async (id: string) => {
      if (!id)
        throw new Error("You must provide a ticket id to this function.");
    ...
    update: async (
      token: string,
      id: string,
      data: APICoachingTicketPatchData,
    ) => {
      if (!token)
        throw new Error(
          "You must provide an authorization token to this function.",
